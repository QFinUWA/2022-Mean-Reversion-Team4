...
class backtest():
    ...
    def __init__(self, data):
        ...
        missing = set(['high', 'low', 'open', 'close', 'volume']
                      )-set(data.columns)
        if len(missing) > 0:
    ...
    def results(self):
        ...
        print("Net Profit   : {0}".format(
            round(help_funcs.profit(self.account.initial_capital, pc1), 2)))

        pc2 = help_funcs.percent_change(
            self.account.initial_capital, self.account.total_value(final_price))
        print("Strategy     : {0}%".format(round(pc2*100, 2)))
        print("Net Profit   : {0}".format(
            round(help_funcs.profit(self.account.initial_capital, pc2), 2)))
    ...
    def chart(self, show_trades=False, title="Equity Curve"):
        ...
        p = bokeh.plotting.figure(
            x_axis_type="datetime", plot_width=1000, plot_height=400, title=title)
        ...
        p.line(self.data['date'], base_equity,
               color='#CAD8DE', legend_label='Buy and Hold')
        p.line(self.data['date'], self.account.equity,
               color='#49516F', legend_label='Strategy')
            ...
            for trade in self.account.opened_trades:
                try:
                    ...
                    y = self.account.equity[np.where(
                        self.data['date'] == trade.date.strftime("%Y-%m-%d"))[0][0]]
                    if trade.type_ == 'long':
