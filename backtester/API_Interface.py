...
# Calculates the timeslices needed for the start and end dates provided
def calculate_time_slice(start_date, end_date):

    # Creates datetime object from start date
    start_datetime_object = datetime.strptime(start_date, '%d-%m-%Y')
    # Creates datetime object from end date
    end_datetime_object = datetime.strptime(end_date, '%d-%m-%Y')
    if start_datetime_object > end_datetime_object:  # If start date is after end date, swap them
        # swaps the objects
        start_datetime_object, end_datetime_object = end_datetime_object, start_datetime_object
        start_date, end_date = end_date, start_date  # swaps the dates

    # Creates datetime object for today
    today = datetime.combine(date.today(), datetime.min.time())
    # Calculates the number of months between today and start date
    monthsagostart = (today - start_datetime_object).days // 30
    if monthsagostart < 1:  # If start date is before today, set start date to today
        ...
        # Calculates the number of years between today and start date
        yearsagostart = (monthsagostart // 12) + 1
        if yearsagostart == 2:
            monthsagostart = monthsagostart - 12  # If the start date is in the year "2",
        timeslicestart = "year" + \
            str(yearsagostart) + "month" + str(monthsagostart)
...
def time_controller(start_date, end_date):
    ...
    if start_date in timeslicelist and end_date in timeslicelist:
        ...
        start_date = datetime.today() - \
            timedelta(
                days=((int(timeslicestart[4])-1)*12 + int(timeslicestart[10:12]))*30)
        end_date = datetime.today() - \
            timedelta(
                days=((int(timesliceend[4])-1)*12 + int(timesliceend[10:12]))*30)
    else:
        timeslicestart, timesliceend, start_date, end_date = calculate_time_slice(
            start_date, end_date)
...
# Maybe rename if intraday is the only one used
def get_intraday_extended(symbol, start_date, end_date, interval, combine=True):
    try:
    ...
    startindex, endindex, start_date, end_date = time_controller(
        start_date, end_date)  # Generate start and end index for retrieving data
    ...
    for i in range(startindex, endindex + 1):
        ...
        if combine == True:
            ...
            df.to_csv("data" + "/" + symbol + '_' +
                      timeslice + '_' + interval + '.csv')
    if combine == True:
        combined_data.reset_index()
        combined_data = combined_data.sort_values(by='date')
        combined_data.reset_index(drop=True, inplace=True)
        combined_data.to_csv("data" + "/" + symbol + '_' + str(start_date.date()) +
                             '_' + str(end_date.date()) + '_' + interval + '.csv', index=False)
