...
def move_averages(value):
    window.append(value)
    if len(window) == mv_av_slow:
        rm = window.pop(0)
        slow = mv_av_slow - (rm + value)/mv_av_slow_size
        fast = mv_av_fast - (window[-mv_av_fast_size] + value)/mv_av_fast_size
        return (slow, fast)
    return (-1, -1)
...
def logic(account, lookback):  # Logic function to be used for each time interval in backtest
    ...
    slow_av, fast_av = move_averages(lookback['close'][today])

    closing.append(lookback['close'][today])
    slow.append(slow_av)
    fast.append(fast_av)
    x.append(counter)
    counter += 1
