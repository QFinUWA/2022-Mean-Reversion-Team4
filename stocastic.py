...
def logic(account, lookback): # Logic function to be used for each time interval in backtest 
    ...
    if(current_period > training_period):
        ...
        if current_ratio <= 0.21*ratio and account.buying_power >0 and current_ratio >= 0.1*ratio:
            Long(account, close) # Enter a long position
...
if __name__ == "__main__":
    ...
    list_of_stocks_proccessed = list_of_stocks #preprocess_data(list_of_stocks) # Preprocess the data
